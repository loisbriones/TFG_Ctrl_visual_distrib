...
from rclpy.parameter import ParameterEventHandler
...
class ArduinoBridgeNode(Node):
    ...
    def on_calibration_mode_change(self, p):
        ...
        if is_calibrating:
            self.get_logger().info(f"Detectado MODO CALIBRACIÓN. Iniciando marcha lenta: {self.calib_speed}")
            self.arduino.set_rail_speed(self.rail_id, self.calib_speed)
        else:
            self.get_logger().info("Calibración terminada. Deteniendo para esperar control dinámico.")

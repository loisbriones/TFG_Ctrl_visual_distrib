...
%   - Se sustituye la tabla vacía tab:vs-personas-ocho por dos tablas,
...
%     REVISAR que esta numeración case con la de las tablas del óvalo.
% =====================================================================

\chapter{Pruebas}
\label{cap:pruebas}

\lettrine{E}{n} este capítulo se recogen las pruebas realizadas sobre el sistema ya montado. La primera cuantifica la diferencia de comportamiento entre los dos coches con una misma configuración. La segunda ajusta el umbral de derrape del algoritmo y mide qué se gana con ello. La tercera enfrenta al algoritmo con pilotos humanos, en el óvalo y en el circuito en ocho.
...
Como se comento en la prueba anterior durante la realización de pruebas unitarias se detectaron errores, en este caso está prueba se realizo porque también se detectaron problemas en el comportamiento del circuito. En días distintos los valores o las configuraciones aplicadas un día para un coche ya no era suficientes la día siguiente. Para demostrar este problema lo que hicimos fue utilizar una misma pista y dejar un pwm incremental durante 10 vueltas. Para este caso igual que el anterior para que no influyeran las latencias de red lo que hicimos fue lanzar todos los nodos en un mismo ordenador de forma que la red no influyera en nada sobre el control del coche. Y como queriamos aplicar un pwm constante se aplico otro modo de funcionamiento al controlador explicado en la seccion~\ref{subsec:modos-operacion}. 

Se realizaron las pruebas en días distintos para demostrar como teniendo el mismo circuito en días distintos por culpa de tener que mover el circuito se detecto que no siempre se puede controlar igual. Como se puede ver en la primera gráfica hacer referencia (), si nos fijamos para un valor de 88 el coche azul ya empieza a perder el control algo que podíamos observar durante en las pruebas presentadas. Pero en cambio otro día por culpa de como se montó la pista el coche azul era capaz de aguantar hasta los 110 de PWM sin presentar derrapes graves. Y lo mismo para el coche rojo en días distintos presenta comportamientos distintos pudiendo soportar más potencia. 

En las gráficas además se puede ver como para el mismo circuito incluso incrementando el valor del PWM no estamos consiguiendo una mejora en el tiempo. Por tanto el problema del circuito no solo influye en que hay que aplicar un PWM mayor ni si quiera aplicando un PWM mayor somos capaces de conseguir los mismo tiempos y además por culpa de esto el coche que nos acaba saliendo, por tanto el buen montaje también influye en el tiempo que podemos conseguir.

Seguramente se deba a el montaje de las pistas a que hay que hacer bien el contacto entre los carriles o que al final son piezas mezcladas de varios tipos distintos de Scalextric lo que puede provocar que el comportamiento del coche no sea el esperado lo que hace más complicado determinar el comportamiento del coche en días distintos. Por eso que para el cargar una politica optima por ejemplo hay que tener cuidado porque puede ocurrir que un día funcione pero si montamos el Scalextric de otra forma no funcionaría. Nosotros es algo que podemos evitar con el algoritmo porque podemos relanzarlo o cambiar parametros en caliente. Aunque no sería la mejor solución este es uno de los problemas que detectamos al algoritmo no es capaz de adaptarse del todo al problema que puede tener la pista. Si el algoritmo fuera lo suficientemente bueno sería capaz de incrementar más su valor tope, pero es algo que durante el diseño no contemplamos porque no esperamos ese comportamiento al no estar reportado. 
% TERMINA REVISION
...
Hasta ahora la mayoría de pruebas que hicimos se hicieron todas dentro de un mismo ordenador para evitar problemas esto se hizo así para evitar pequeñas latencias qeu podía incluir la red. Durante esta sección vamos a realizar varias pruebas, para demostrar que el retardo normal no influye aunque durante las pruebas realizadas se hicieron casi todas por wifi y nunca hubo problemas. Se van a realizar en 3 escenarios distintos uno donde todo este dentro del mismo ordenador, otro donde tenemos el nodo camara conectadod por cable al router y el ordenador conectado por cable al router. Y después el mismo caso pero donde todo está conectado por wifi. Para el caso entendemos que el nodo que va a tener los controladores tiene que ser un nodo que tenga bastante potencia para tener todos los controladores corriendo para todos los coches y para también eliminar retardo de llegada entre el controlador y aplicar la tensión asumimos que el nodo controlador va a estar conectado aunque en caso de necesidad se podría tener en otro nodo corriendo. 

Además también para ver hasta qeu punto la latencia afecta al control vamos a utilizar una herramienta que añade o tira paquetes para ver que pasaría si estuvieramos en un escenario complicado.

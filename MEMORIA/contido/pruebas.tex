...
De cada ejecución de este capítulo se grabó un vídeo con la imagen de las cámaras sincronizada y los datos del control sobrepuestos, y todos están publicados. Las tablas de resultados llevan por eso una columna con el enlace al vídeo de cada fila, de manera que cualquier cifra se puede contrastar viendo lo que hizo el coche en la pista.
...
\begin{figure}[tbp]
  ...
  \caption{Montaje del circuito en óvalo. La única cámara va sujeta al pórtico que se ve sobre el trazado, y a la derecha queda el ordenador con el nodo de control y el puente Arduino}
...
\begin{table}[tbp]
  ...
  \begin{tabular}{lrrrrrrrr}
    ...
    % Aquí las filas no son ejecuciones sino escalones de PWM, así que el enlace
    % al vídeo va en la cabecera de cada grupo de columnas, que sí lo es.
    \rowcolor{ArqAzulClaro}
...
\begin{figure}[tbp]
  ...
  \caption{Montaje del circuito en ocho, cubierto por dos cámaras repartidas entre dos máquinas. Se ven los soportes de aluminio desde los que enfocan el trazado}
...
El objetivo de esta prueba es comprobar cuanto retardo añade la red al lazo de control y a partir de qué punto llega a notarse en la pista. Se hizo sobre el óvalo de la figura~\ref{fig:setup-ovalo} con una sola cámara, que es el trazado del que más datos se tienen y ser más sencillo de trabajar.
...
\begin{table}[tbp]
  ...
  \label{tab:red-rtt}
  % Cada fila resume las TRES repeticiones del escenario, así que la celda del
  % vídeo lleva los tres enlaces numerados en vez de un único «ver».
  \begin{tabular}{lrrrrrc}
    \toprule
    \rowcolor{ArqAzulClaro}
...
El resto del capítulo trabaja sobre trazados pequeños, que caben enteros en una cámara o en dos. Esta prueba lleva el sistema a circuitos grandes, del tamaño con el que se juega de verdad, para comprobar que la arquitectura sigue funcionando cuando hace falta repartir el trabajo entre varias máquinas, que es para lo que se diseñó. Se montaron dos trazados, y el segundo es mayor que el primero. Los dos se cubrieron con cuatro cámaras: dos de ellas en sendas Raspberry Pi 5 conectadas por WiFi, y las otras dos en el propio ordenador, donde también se ejecutan el controlador del coche y el nodo puente que habla con el Arduino.

En cada circuito se hicieron ejecuciones con el algoritmo y ejecuciones de conducción manual, de 50 vueltas todas ellas, de manera que aquí se comprueban dos cosas a la vez: que el sistema escala a un trazado grande repartido entre varias máquinas, y que sobre él el algoritmo sigue por delante de una persona.

\subsection{Primer circuito}
\label{subsec:circuito-grande-1}

El trazado y el reparto de las cuatro cámaras se ven en la figura~\ref{fig:setup-grande-1}. Sobre este montaje se hicieron dos ejecuciones automáticas seguidas, una con cada coche, con un rango de PWM de 65 a 85. Entre una y otra no se movió ninguna cámara ni se cambió nada más que el umbral de derrape, 12~px en el azul y 13~px en el rojo. Al día siguiente, con el circuito y las cámaras sin tocar, el piloto 1 dio sus 50 vueltas con cada coche.
...
\begin{figure}[tbp]
  ...
  \end{subfigure}
  \caption{Montaje del primer circuito grande, cubierto por cuatro cámaras repartidas entre tres máquinas. Se ven los soportes de aluminio y los trípodes desde los que enfocan el trazado}
  \label{fig:setup-grande-1}
\end{figure}

Lo primero que hay que decir es que las dos ejecuciones automáticas terminaron sus 50 vueltas sin que nadie tuviera que intervenir, con las cuatro cámaras repartidas entre tres máquinas y sin más configuración que añadirlas a la lista de cámaras. La figura~\ref{fig:circuito-grande-tiempos} recoge el tiempo de cada vuelta. Las dos curvas hacen lo mismo que en el resto de las pruebas, bajan durante las primeras vueltas mientras el algoritmo sube el perfil y después se estabilizan, solo que el azul parte de una primera vuelta de 6,05~s y necesita diecisiete para alcanzar su tiempo de crucero, mientras que el rojo arranca en 4,51~s y llega en doce.

\begin{figure}[tbp]
  \centering
  \includegraphics[width=\linewidth]{imaxes/PRUEBAS/CIRCUITO_GRANDE/tiempos_vuelta.pdf}
  \caption{Tiempo por vuelta de las dos ejecuciones automáticas sobre el primer circuito grande.}
  \label{fig:circuito-grande-tiempos}
\end{figure}

Los tiempos de la tabla~\ref{tab:circuito-grande-tiempos} son los esperados. El coche rojo completa las 50 vueltas en 188,85~s frente a los 212,48~s del azul, casi 24~segundos de diferencia, y además es mucho más regular, con una $\sigma$ de 0,257~s frente a 0,540~s. Es el mismo resultado que ya daba la sección~\ref{sec:prueba-comparativa-coches} sobre el óvalo y con una sola cámara, así que cambiar de trazado y repartir el circuito entre cuatro cámaras no altera el orden entre los coches: lo que separa a uno del otro sigue siendo el coche.
...
\begin{table}[tbp]
  \centering
  \estilotabla[3]
  \caption{Tiempos de las 50 vueltas sobre el primer circuito grande. Las ejecuciones automáticas van con PWM 65--85.}
  ...
  \begin{tabular}{llrrrrrc}
    ...
    Ejecución & Coche & Contrarreloj & Mejor & Mediana & Media & $\sigma$ & Vídeo \\
              \rowcolor{ArqAzulClaro} & & (s) & (s) & (s) & (s) & (s) & \\
    \midrule
    % PENDIENTE-VIDEO: CIRCUITO_FINAL_ROJO_001
    Algoritmo         & Rojo & 188,85 & 3,449 & 3,715 & 3,777 & 0,257 & \\
    % PENDIENTE-VIDEO: CIRCUITO_FINAL_AZUL_001
    Algoritmo         & Azul & 212,48 & 3,805 & 3,992 & 4,250 & 0,540 & \\
    % PENDIENTE-VIDEO: CIRCUITO_FINAL_ROJO_MANUAL_001
    Manual (piloto 1) & Rojo & 274,18 & 3,148 & 4,535 & 5,484 & 2,754 & \\
    % PENDIENTE-VIDEO: CIRCUITO_FINAL_AZUL_MANUAL_001
    Manual (piloto 1) & Azul & 317,03 & 4,434 & 5,321 & 6,341 & 2,627 & \\
    \bottomrule
  \end{tabular}
\end{table}

Durante las dos ejecuciones automáticas siguió corriendo el nodo de sondeo de la sección~\ref{sec:pruebas-red}, midiendo el tiempo de ida y vuelta hasta cada una de las dos Raspberry. La tabla~\ref{tab:circuito-grande-red} recoge esas medidas. Las medianas se quedan entre 6,90 y 7,88~ms, prácticamente las mismas que allí se midieron por WiFi (7,27~ms), y de los 4.306 sondeos enviados solo se perdió uno. Lo que sí sube aquí es la cola de la distribución, con percentiles 99 que llegan a los 135~ms y algún pico suelto por encima de los 190. Es razonable, porque ahora hay cuatro cámaras publicando su imagen de depuración y la sesión entera grabándose en disco, una carga que ninguna de las ejecuciones de la sección anterior tenía encima. Con los datos que se tienen no se puede repartir la culpa entre el enlace WiFi y la máquina, pero sí se ve que los picos son sueltos y que la mediana no se mueve.

\begin{table}[tbp]
  \centering
  \estilotabla[3]
  \caption{Tiempo de ida y vuelta hasta cada una de las dos Raspberry Pi durante las dos ejecuciones automáticas del primer circuito.}
...
Aun con esos picos, en la pista no se notó nada, y encaja con lo que ya decía la sección~\ref{sec:pruebas-red}: allí hubo que inyectar 100~ms de retardo a mano, más de diez veces el que se mide aquí, para empezar a ver una diferencia en los tiempos, y ni siquiera entonces el coche perdía el control.

\subsection{Segundo circuito}
\label{subsec:circuito-grande-2}

El segundo trazado, que aparece en la figura~\ref{fig:setup-grande-2}, es más largo que el primero, y se montó el mismo día por la tarde con el mismo reparto de cámaras y máquinas. La diferencia se nota en la calibración: la trayectoria base pasa de 144 a 177 nodos sumando las cuatro cámaras, y a cada una de ellas le toca una porción mayor del trazado. Aquí las ejecuciones automáticas no comparten configuración, porque el circuito se estrenó ese mismo día y hubo que buscar el rango de PWM de cada coche por separado: 65 a 78 en el azul y 70 a 90 en el rojo, las dos con el umbral de derrape en 10~px. Por eso esta tabla no compara los dos coches entre sí, cosa que ya hace la del primer circuito; lo que compara es cada coche con las personas que lo condujeron.
...
\begin{figure}[tbp]
  ...
  \end{subfigure}
  \caption{Montaje del segundo circuito grande, más largo que el primero y con el mismo reparto de cuatro cámaras entre tres máquinas}
  \label{fig:setup-grande-2}
\end{figure}

La figura~\ref{fig:circuito-grande-2-tiempos} muestra el tiempo por vuelta de las dos ejecuciones automáticas y se lee igual que la del primer circuito, con la bajada inicial mientras el perfil sube y la estabilización después.
...
Los resultados están en la tabla~\ref{tab:circuito-grande-2-tiempos} y el algoritmo gana las dos comparaciones con holgura. Con el coche rojo cierra en 217,56~s frente a los 358,27~s del piloto 1, un 65\,\% más para la persona, y con el azul en 322,41~s frente a los 458,51 y 536,30~s de los pilotos 1 y 5. Aquí, además, ni siquiera queda el consuelo de la vuelta rápida: la mejor vuelta es del algoritmo en los dos coches, 4,129~s frente a 4,355~s en el rojo y 5,636~s frente a 6,076~s en el azul. La diferencia de fondo es la de siempre y se ve en la última columna, porque la desviación típica del algoritmo se queda en 0,238 y 0,939~s mientras que la de los pilotos no baja de 2,7~s.

\begin{table}[tbp]
  \centering
  \estilotabla[3]
  \caption{Tiempos de las 50 vueltas sobre el segundo circuito grande. Las ejecuciones automáticas van con PWM 65--78 en el azul y 70--90 en el rojo, y umbral de derrape de 10~px las dos.}
  ...
  \begin{tabular}{llrrrrrc}
    ...
    Ejecución & Coche & Contrarreloj & Mejor & Mediana & Media & $\sigma$ & Vídeo \\
              \rowcolor{ArqAzulClaro} & & (s) & (s) & (s) & (s) & (s) & \\
    \midrule
    % PENDIENTE-VIDEO: CIRCUITO_FINAL_ROJO_003
    Algoritmo         & Rojo & 217,56 & 4,129 & 4,250 & 4,351 & 0,238 & \\
    % PENDIENTE-VIDEO: CIRCUITO_FINAL_AZUL_002 (el de BAGS_GUARDAR, PWM 65-78)
    Algoritmo         & Azul & 322,41 & 5,636 & 6,281 & 6,448 & 0,939 & \\
    % PENDIENTE-VIDEO: CIRCUITO_FINAL_ROJO_MANUAL_002_LOIS
    Manual (piloto 1) & Rojo & 358,27 & 4,355 & 5,684 & 7,165 & 3,009 & \\
    % PENDIENTE-VIDEO: CIRCUITO_FINAL_AZUL_MANUAL_002_LOIS
    Manual (piloto 1) & Azul & 458,51 & 6,076 & 7,907 & 9,170 & 2,720 & \\
    % PENDIENTE-VIDEO: CIRCUITO_FINAL_AZUL_MANUAL_002_MANU
    Manual (piloto 5) & Azul & 536,30 & 6,093 & 8,007 & 10,726 & 6,556 & \\
    \bottomrule
  \end{tabular}
\end{table}

El sondeo de red se mantuvo también en estas dos ejecuciones y sus medidas están en la tabla~\ref{tab:circuito-grande-2-red}. Salen prácticamente iguales que las del primer circuito, con medianas entre 6,70 y 7,86~ms y la misma cola de percentiles 99 por encima de los 100~ms, y de los 5.826 sondeos enviados se perdieron cuatro. Que un trazado mayor no mueva estas cifras tiene sentido, porque lo que viaja por el enlace es una posición por fotograma y eso no depende de lo largo que sea el circuito.
...
En resumen, el sistema se comportó como estaba pensado en los dos circuitos. Cuatro cámaras repartidas entre tres máquinas cubren un circuito grande, cada una aprende su porción del trazado, el controlador va conmutando entre ellas según por dónde pase el coche y el algoritmo ajusta el perfil de la misma forma que con una sola cámara. Escalar el montaje no exigió tocar ningún parámetro del algoritmo ni ninguna otra parte del sistema, y la red no fue un problema en ninguna de las cuatro ejecuciones automáticas.

Frente a los pilotos el algoritmo gana las cinco comparaciones, y por márgenes mayores que en el óvalo y el circuito en ocho de la sección~\ref{sec:prueba-algo-vs-personas}: allí la distancia iba del 2 al 36\,\% según el coche y el trazado, y aquí del 42 al 65\,\%. El motivo es el que ya se explicó en esa sección y no hace falta repetirlo, solo que aquí se ve más: cuanto más largo es el trazado, más sitios hay donde perder el coche y más vueltas cuesta cada error, mientras que el algoritmo se limita a repetir el mismo tiempo cincuenta veces.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrc}
    ...
    \midrule
    % PENDIENTE-VIDEO: PRUEBA_CIRCUITO_OCHO_002
    Sin peraltes & 141,36 & 2,665 & 2,832 & \\
    % PENDIENTE-VIDEO: PRUEBA_CIRCUITO_OCHO_002_peraltes
    Con peraltes & 136,53 & 2,571 & 2,728 & \\
    \bottomrule
  \end{tabular}
\end{table}

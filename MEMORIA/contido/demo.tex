...
(páxina~\pageref{tab:longa}), recoméndase o uso do paquete \lstinline{lontable},
...
Para isto é preciso o paquete \lstinline{multirow}, incluído entre os recomendados no
...
Para usar o estilo anterior en palabras ou expresións curtas insertadas nun
parágrafo, recoméndase facelo co comando \lstinline{lstinline}.
Nel úsase un tamaño de letra máis coherente co do resto do texto
(\lstinline{small}), frente ao tamaño de letra usado no \lstinline{lstlisting}
({\tt \footnotesize footnotesize}).

\section{Uso da relación de acrónimos e do glosario}

Os acrónimos edítanse no ficheiro \lstinline{bibliografia/acronimos.tex}
e úsanse empregando a orde \lstinline{acrlong} para obter o termo
completo (deste xeito: \acrlong{erlang}), a orde \lstinline{acrshort}
...
\lstinline{acrfull} (que produce ambas versións á vez:
\acrfull{erlang}). Os acrónimos que non se usan no documento, non
aparecen na relación que se xerar na versión PDF.

Pola súa banda, os termos do glosario edítanse en
\lstinline{bibliografia/glosario.tex} e úsanse empregando a orde
\lstinline{gls} (deste xeito, \gls{bytecode}) ou \lstinline{Gls} (deste

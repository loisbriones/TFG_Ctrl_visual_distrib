...
Para poder realizar el control es necesario saber donde están los dos ejes del coche, tanto el delantero como el trasero, con respecto a la trayectoria base, (sección~\ref{subsec:trayectoria-base}). Razón por la cual, para cada una se obtiene la celda más cerca y su distancia a ella.

Después cada una tiene un uso distinto. La distancia de la delantera se debe comprobar para asegurar que el coche sigue el recorrido esperado, si supera un umbral se considera que la detección no es válida y el fotograma se descarta sin llegar a calcular un PWM nuevo. La distancia de la trasera es la señal con la que se detectan los derrapes, como se detalla en la sección~\ref{subsec:algoritmo-derrapes}.
...
La sección~\ref{subsec:derrape-arquitectura} exige que exista una señal capaz de indicar en qué punto del circuito el coche ha perdido el control, es decir, derrapado. La señal escogida aquí es la distancia perpendicular de la pegatina trasera a la trayectoria base, seccion~\ref{subsec:trayectoria-base}. Cuando el eje trasero del coche se abre, esa distancia crece, porque el coche está empezando a derrapar.
...
La política de castigo se dispara al cerrarse un derrape. El intervalo de celdas en las que se detectó se extiende hacia atrás una distancia fija en píxeles, que es el tramo de anticipación, y el resultado pasa a ser una zona de derrape con un PWM igual al mínimo de los de las zonas que cubría menos dos unidades, con el parámetro \texttt{minimum\_speed} (sección~\ref{subsec:params-yaml}) como suelo. Si el intervalo toca una zona de derrape ya existente ambas se fusionan, con la unión de los dos intervalos y el menor de los dos PWM menos la reducción; el retroceso que se añade al fusionar es más corto, para que reincidir en una misma curva no acabe extendiendo la zona sobre el circuito entero. Las celdas gigantes que alcance el castigo se rebajan enteras.

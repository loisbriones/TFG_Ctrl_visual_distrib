...
\section{Implementación de la arquitectura en ROS 2}
\label{sec:arquitectura_ros2}

El sistema esta formado por un conjunto de nodos de ROS~2 repartidos entre varias máquinas, que se comunican publicándose y suscribiéndose a \emph{topics}. Para lograr el control del coche se utiliza un Arduino junto con un L293D conectados a la pista. La figura~\ref{fig:arquitectura-ros2} recoge el despliegue completo y el cuadro~\ref{tab:topics-ros2} resume los \emph{topics} implicados.
...
Cada \textbf{nodo de captura} lleva asociada una cámara física, detecta las pegatinas de los coches que pasan por su porción del circuito y publica una posición por coche y fotograma. Esas posiciones las consume el \textbf{nodo controlador} del coche correspondiente, que reconstruye durante la calibración la trayectoria que este sigue, contabiliza los pasos por meta y, en carrera, traduce cada posición en un valor de PWM. Los valores de PWM que hay que aplicar convergen en el \textbf{nodo puente}, único en el sistema y único con acceso al hardware, que las traduce al protocolo serie del Arduino. En paralelo, el \textbf{sistema de visualización} escucha todo lo que circula por la red y lo almacena, sin intervenir en el lazo de control.

Salvo el puente, todos los tipos de nodo se instancian tantas veces como haga falta. En el caso de los nodos camara deberíamos de tener tantos como necesitemos para cubrir el circuito, y después deberíamos de tener un controlador por cada coche que queramos controlar. Para evitar choques entre los topics y para simplificar la creación de los mismos, cada uno de las instancias arranca dentro de su propio espacio de nombres (sección~\ref{sec:ros2-namespaces}), ligando el nodo a los topics necesarios para el correcto funcionamiento.
...
\hline
\texttt{/carN/modo\_calibracion} & \texttt{Bool} & Controlador $N$ & Cámaras, controladores y puente \\
\hline
...
A continuación se definen los mensajes creados en ROS~2 para modelar la información que tienen que intercambiar los nodos para el correcto funcionamiento, \ref{sec:modelo-datos}. Para lograrlo, fue necesario crear mensajes propios (\ref{sec:ros2-interfaces}) porque los que trae por defecto ROS~2 no valían para modelar todas las exigencias del sistema, además también hubo que determinar que Calidad de Servicio (QoS) se iba a usar en los \emph{topics} (\ref{sec:ros2-topics}) para cumplir con las necesidades de entrega del sistema, cuarta columna en \ref{sec:modelo-datos}.

\subsection{Mensajes propios del sistema}
\label{subsec:mensajes-propios}

El mensaje central del sistema es \texttt{CarLocation}, la \emph{posición del coche} del cuadro~\ref{tab:mensajes}: cada nodo cámara publica uno por coche y por fotograma procesado, y sin él no funcionaría el resto del sistema. Su definición completa se recoge en el listado~\ref{lst:msg-carlocation} y sirve de ejemplo del formato que siguen todos los demás mensajes propios.
...
Los dos primeros campos identifican el origen del dato: qué cámara lo generó y a qué coche corresponde. \texttt{front} y \texttt{back} son las dos pegatinas detectadas, una delantera y otra trasera, cada una con su centroide y su color, y \texttt{bounding\_rect\_stiker\_front} y su equivalente trasero, los rectángulos que las encierran. \texttt{stamp} es la marca de tiempo de captura del fotograma y \texttt{proc\_time} lo que costó procesarlo. \texttt{n\_frame}, por último, es el contador de fotogramas procesados por esa cámara: el mismo número que se superpone a la imagen de depuración y que el algoritmo escribe en su registro, lo que permite cruzar las tres fuentes durante el análisis.

El resto de mensajes de información sigue el mismo formato y su definición se recoge en el anexo~\ref{lst:msgs-informacion}:

\begin{itemize}

\item \texttt{FinishLine} implementa la \emph{línea de meta} del cuadro~\ref{tab:mensajes}. Contiene el identificador de la cámara que la observa y el segmento que la delimita en coordenadas de la camara.

\item \texttt{SpeedCarril} implementa la \emph{consigna de potencia}: el valor de PWM, el identificador del carril sobre el que aplicarlo (en la forma \texttt{rN}) y una marca temporal que permite saber el tiempo de retraso con el que llegan los valores.

\item \texttt{TimePerLap} implementa el \emph{tiempo de vuelta}. El controlador lo publica en cada paso por meta con el tiempo de la vuelta completada, su número y el instante del cruce.

\item \texttt{CarControlTelemetry} no tiene equivalente en el cuadro~\ref{tab:mensajes}: es telemetría del lazo de control que el capítulo~\ref{chap:arquitectura} no recoge, publicada en cada iteración. Incluye la distancia de derrape estimada y si el coche está derrapando, el tiempo de \emph{pipeline} desde que se recibe la posición hasta que se publica el PWM, el instante en que llegó el mensaje de la cámara y el tamaño del \emph{ROI}. Mensaje diseñado durante la implementación para poder realizar analisis del funcionamiento del algoritmo.

\end{itemize}

Junto a ellos hay cuatro mensajes auxiliares, piezas de construcción de los anteriores (anexo~\ref{lst:msgs-auxiliares}): \texttt{Point2D}, un punto en coordenadas de imagen; \texttt{Stiker}, es un \texttt{Point2D} junto con un \emph{string} que indica el color del mismo; \texttt{BoundingRect}, el rectángulo delimitador de una detección, usado para la depuración visual y \texttt{LineSegment}, representa un segmento definido por sus dos extremos.

Los dos mensajes restantes del cuadro~\ref{tab:mensajes} no necesitan un tipo propio y se resuelven con tipos estándar de ROS~2. El \emph{cambio de modo} es un \texttt{std\_msgs/Bool} en el \emph{topic} de calibración de cada coche, con el que su controlador notifica al resto de nodos el fin de esa fase. La \emph{vista de la cámara} es un \texttt{sensor\_msgs/CompressedImage} en los \emph{topics} de depuración, sobre los que cada cámara publica el fotograma procesado con las detecciones superpuestas.

\subsection{Calidad de servicio}
\label{subsec:qos}

El capítulo~\ref{chap:arquitectura} distingue dos grupos de mensajes según su garantía de entrega. Esa distinción se traduce en dos políticas de calidad de servicio (QoS) que ofrece ROS~2 (sección~\ref{sec:dds-qos}).

Los flujos de alta frecuencia, en los que cada mensaje queda obsoleto en cuanto llega el siguiente, emplean el perfil predefinido para datos de sensores, de fiabilidad \emph{best effort}. Lo usan las posiciones de los coches, las consignas de PWM, los dos \emph{topics} de telemetría y las imágenes de depuración. Retransmitir un mensaje perdido solo consigue entregar tarde un dato ya caducado y retrasar de paso el actual.

La línea de meta, que se publica una única vez y en el arranque, emplea la política opuesta: fiabilidad garantizada y durabilidad \emph{transient local} con profundidad uno. Así, un controlador que arranque más tarde que las cámaras la recibe igualmente, para evitar imponer un arranque y la constante publicación. Como los perfiles se negocian entre las dos partes, cada suscripción del sistema replica el del \emph{topic} al que se conecta.

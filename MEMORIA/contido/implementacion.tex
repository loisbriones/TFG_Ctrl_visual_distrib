...
\begin{figure}[tbp]
    ...
    \caption{Trayectoria base para el circuito ovalo pequeño}
...
Frente a las dos herramientas anteriores, que trabajan en diferido, el panel de carrera enseña lo que está pasando mientras se corre. Es un nodo que se suscribe a los mismos \emph{topics} que ya publica el sistema y sirve su contenido por HTTP a un navegador, sin publicar nada a cambio, de modo que puede levantarse y cerrarse en mitad de una carrera sin que el resto se entere. La figura~\ref{fig:panel-directo} recoge una ejecución sobre el circuito en óvalo con una cámara. Arriba quedan la clasificación, con la vuelta en curso, los tiempos, la tensión aplicada y el estado del coche, y el circuito, que no se carga de ningún fichero sino que se va dibujando con las propias posiciones que llegan de la cámara, con la línea de meta marcada en naranja y el coche sobre ella, unido a su etiqueta trasera para que se vea a simple vista cuánto se separa de la trayectoria. Debajo, el registro de eventos deja por escrito lo que va ocurriendo, cada vuelta con su tiempo y los derrapes según se detectan.
...
El número de nodos de control y la existencia de un nodo puente, se decide dentro del launch de forma dinámica justo antes del lanzamiento. Recorre la lista \texttt{coches} del fichero de parámetros y crean un controlador por cada uno, filtrando por su \texttt{modo} de operación (sección~\ref{subsec:modos-operacion}). El principal (anexo~\ref{lst:brain-launch}) se queda con los coches que sí publican tensión —los modos incremental, automático y por política— y añade el nodo puente, que solo tiene sentido si hay que controlar algún coche. El de carrera manual (anexo~\ref{lst:manual-launch}) se queda con los manuales y no arranca el puente, no sé publica el mensaje de potencia \ref{subsec:modelo-datos}. Sus controladores se levantan con el modo manual activo, permite obtener información pero no sé publica ningún valor de voltaje. Que sean dos ficheros y no uno se debe a que el despliegue del principal monta el dispositivo del Arduino, y una carrera enteramente manual debe poder levantarse sin tenerlo conectado.\footnote{Cada controlador recibe un nombre de nodo único derivado del nombre del coche y del modo, porque el controlador genera sus ficheros de \emph{log} con el nombre del nodo y dos nodos homónimos se pisarían.}

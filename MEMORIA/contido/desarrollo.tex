...
Antes de una sesión, el usuario define el comportamiento del sistema editando un único fichero de parámetros: indicar los colores de las pegatinas de cada coche y de la línea de meta, los coches que participan en la carrera, la resolución de la camara, etc. A continuación, arranca cada uno de los equipos de la red local, con lo que los nodos se descubren automáticamente y comienzan la fase de calibración. Para arrancar los nodos habría que utilizar un comando de docker, habría que hacerlo en cada uno de los dispositivos que formen parte de la red.
%Se podría hacer un script para automatizar despliegue con ansible???

Durante la ejecución, el usuario  es el encargado de detener el sistema cuando lo considere necesario. En caso de activarlo durante la carrera puede ver lo que esta pasando en el nodo de visualización, aunque puede afectar al rendimiento. Una vez terminada la carrera, si activo el guardado de la información puede analizarla en diferido.
...
Tal y como se planteó en el anteproyecto, se ha seguido una metodología ágil e incremental. El trabajo comenzó con una fase de estudio y análisis, en la que se revisaron los TFG anteriores para identificar puntos de mejora y se evaluaron las tecnologías disponibles. A continuación se abordó el diseño de la arquitectura, la implementación incremental de las mejoras y su validación en entornos reales, para concluir con la documentación y la preparación de la defensa.

El desarrollo se organizó en iteraciones cortas, cada una orientada a incorporar una capacidad concreta al sistema. Una iteración no se daba por cerrada hasta validar su resultado sobre el hardware real, de modo que las pruebas de cada etapa condicionaban el diseño de la siguiente. Este enfoque resultó adecuado en un proyecto con una fuerte componente hardware, donde el comportamiento del sistema difícilmente puede anticiparse sin ensayos sobre el circuito.

Al tratarse de un proyecto individual, no se aplicaron las ceremonias propias de marcos como Scrum. El seguimiento se articuló mediante reuniones periódicas con el director, en las que se revisaba la iteración en curso y se acordaban los objetivos de la siguiente. El código se gestionó con un sistema de control de versiones.
...
Con el fin de garantizar la trazabilidad y la reproducibilidad del trabajo, todos los artefactos generados durante el desarrollo del proyecto se han organizado y publicado en un repositorio de acceso público alojado en GitHub. Dicho repositorio reúne el código fuente de los nodos ROS2 escritos en Python, las definiciones de los mensajes personalizados intercambiados entre nodos, el firmware de Arduino encargado de generar las señales PWM, así como los ficheros de configuración del sistema (el fichero de parámetros en YAML y los ficheros de Docker Compose empleados en el despliegue).

Para ilustrar el funcionamiento del sistema se han grabado y publicado
vídeos de las distintas pruebas realizadas capturadas apartir del ros bag~\ref{sec:ros2-rosbag}, accesibles desde el canal de YouTube asociado al proyecto. Este material aporta evidencia visual de los resultados alcanzados.
...
El material hardware necesario para el desarrollo y la puesta en funcionamiento del sistema fue el siguiente:
...
Sumando el coste de los recursos hardware (tabla~\ref{tab:costes_hardware}) y el de los recursos humanos (tabla~\ref{tab:costes_humanos}), y teniendo en cuenta que los recursos software no suponen coste alguno, el coste total estimado del proyecto asciende a\textbf{27\,411{,}21 \euro}.
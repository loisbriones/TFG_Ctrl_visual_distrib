...
\subsection{Nodos}
Procesos independientes que encapsulan una funcionalidad concreta (en nuestro caso, la gestión de una cámara, el control de un coche o el puente con el Arduino) y que pueden ejecutarse en máquinas distintas.

\subsection{Topics y mensajes}
Bus que permite la comunicación entre los nodos que publican y reciben mensajes tipados. En el proyecto se definen tipos de mensajes propios para intercambiar la posición del coche, el valor del pwm ("velocidad") y telemetría.

\subsection{Parámetros}
Valores de configuración declarados por cada nodo, que pueden establecerse desde un fichero \gls{yaml} común y modificarse en caliente durante la ejecución.

\subsection{Ficheros Launch}
Scripts de lanzamiento que permiten arrancar varios nodos junto con sus configuraciones 
    
\subsection{Namespaces}
Es un mecanismo de encapsulación jerárquica que agrupa identificadores del sistema (nodos, tópicos, servicios o parámetros) bajo un prefijo de ruta común, es un sistema análoga a los directorios de un sistema de ficheros.

\subsection{Executors y callbacksgroups}
Mecanismos de planificación que permiten atender varios \textit{callbacks} en paralelo dentro de un nodo. Se emplea el \texttt{MultiThreadedExecutor} junto con grupos mutuamente excluyentes para separar tareas críticas (procesamiento de posición) de tareas secundarias (envio de imagen para depuración).


\subsection{Colcon}
Colcon es la herramienta oficial de construcción de "workspaces" de ROS2~\cite{ros2docs}.  Se encarga de compilar el paquete del proyecto, generar el código de los mensajes personalizados a partir de sus definiciones e instalar los ejecutables y ficheros de configuración para que puedan ser lanzados con las utilidades de ROS2.
